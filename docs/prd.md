...
* **处理逻辑**：
    ...
    4. 如果是跨域登录请求（OAuth2流程），生成 `Authorization Code` 并重定向回业务系统。
* **输出**：SSO Cookie 及 重定向指令。

#### F2. 令牌签发 (Token Issue)
* **内容**：JWT Payload 中**不包含**具体的权限列表（因 128位 Mask 对应的字符串较短，可视情况放入 Payload 或单独接口获取）。
* **Payload 规范**：
```json
{
  "uid": "uuid-xxx",
...
* **逻辑**：清除根域名 Cookie，并（可选）将当前 JWT 加入 Redis 黑名单，通知所有订阅的子系统 Session 失效。
...
* **步骤**：
    1. 解析请求 Cookie 中的 JWT，提取 UserID。
    2. 查询该用户的 Mask（优先查本地 Redis 缓存，无缓存查鉴权中心 `/api/user/mask`）。
    3. 确定当前 API 需要的权限（如 `user:edit`）。
    4. **位运算判定**：
    ```go
    // Go 伪代码
